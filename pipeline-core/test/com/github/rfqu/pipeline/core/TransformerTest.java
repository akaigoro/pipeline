...
public class TransformerTest {
    ...
    class PipelineGenerator1 extends PipelineGenerator{
    	void make() throws InterruptedException, ExecutionException {
            source=new CharBufSource();
            MyTransformer tf=new MyTransformer();
            tf.setSource(source);
            sink=new CharBufSink();
            sink.setSource(tf);
    	}
    ...
    class PipelineGenerator2 extends PipelineGenerator {
    	void make() throws InterruptedException, ExecutionException {
            ...
            MyTransformer tf=new MyTransformer();
            tf.setSource(source);
            MyTransformer tf2=new MyTransformer();
            tf2.setSource(tf);
            sink=new CharBufSink();
            sink.setSource(tf2);
    	}
    ...
    class MyTransformer extends Transformer<CharBuffer, CharBuffer> {
    	private static final int capacity = 10;

		{
    		super.mySinkPort=new PausableInp();
    		super.myReturnPort=new PausableOut();
    		
    		for (int k=0; k<2; k++) {
        		CharBuffer buf=CharBuffer.allocate(capacity);
    			super.myReturnPort.post(buf);
    		}
    	}

		@Override
		protected void transform(CharBuffer inmessage, CharBuffer outmessage) {
			char ch=inmessage.get();
			outmessage.put(ch);			
		}
    }
}
